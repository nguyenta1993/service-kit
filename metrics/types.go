...
func (m *Monitor) AddMetric(metric *Metric) {
	if _, ok := m.metrics[metric.Name]; ok {
		logger.Error("metric is existed", zap.String("metric_name", metric.Name))
	}

	if metric.Name == "" {
		logger.Error("metric must have name")
	}
	...
	logger.Error("metric type not existed.", zap.String("metric type", fmt.Sprintf("%d", metric.Type)))
}
